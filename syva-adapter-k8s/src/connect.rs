...
pub async fn connect_with_retry(socket_path: &str, max_attempts: usize) -> anyhow::Result<SyvaCoreClient<Channel>> {
        ...
        match connect_to_core(socket_path).await {
            ...
            Err(e) => {
                if attempt == max_attempts {
                    return Err(anyhow::anyhow!("failed to connect to syva-core after {max_attempts} attempts: {e}"));
                }
