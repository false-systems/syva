...
SyvaZonePolicy CRDs ─► syva-k8s ─┤── Unix socket (syva.core.v1) ──► syva-core ──► 9 LSM hooks
...
- **Syva attaches nine BPF-LSM hooks:** `file_open`, `bprm_check_security`, `ptrace_access_check`, `task_kill`, `mmap_file`, `unix_stream_connect`, `socket_connect`, `socket_sendmsg`, `socket_bind`. cgroup-movement / zone-escape protection is still **not BPF-LSM enforceable** — `cgroup_attach_task` is not a BPF-LSM hook on mainline kernels; do not reintroduce it (see the cgroup-escape detection mechanism instead).
...
- **No trait objects where enums suffice.** The hook set is fixed (nine hooks); policy types are known at compile time. Use enums. `dyn Trait` on a hot path pays a vtable cost for flexibility you don't need.
