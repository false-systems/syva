...
pub fn enumerate_cgroups(
    ...
    for root in &cgroup_roots {
        ...
        scan_cgroup_dir(root_path, zone_names, &mut assignments, 0)?;
    }
...
fn scan_cgroup_dir(
    ...
    for entry in entries.flatten() {
        ...
        scan_cgroup_dir(&path, zone_names, assignments, depth + 1)?;
    }
...
pub async fn watch_containerd_events(
    socket_path: String,
    zone_names_rx: tokio::sync::watch::Receiver<Arc<HashSet<String>>>,
    ...
    loop {
        match try_watch(&socket_path, &zone_names_rx, &tx).await {
            Ok(()) => {
...
async fn try_watch(
    ...
    while let Some(envelope) = stream.message().await? {
        ...
        if topic == "/tasks/start" {
            let zone_names = zone_names_rx.borrow().clone();
            handle_task_start(event, &zone_names, tx).await;
...
async fn handle_task_start(
    ...
    if let Err(e) = tx
        ...
        .await
    {
        if let WatcherEvent::Add(a) = e.0 {
            tracing::error!(container = a.container_id, "zone assignment channel closed — enforcement loop is dead");
        }
    }
}
...
async fn handle_task_delete(
    ...
    if let Err(e) = tx
        ...
        .await
    {
        if let WatcherEvent::Remove { container_id, .. } = e.0 {
